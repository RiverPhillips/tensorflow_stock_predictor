...
if data_source == 'alpha_vantage':
    ...
    df = pd.read_csv(os.path.join('Stocks', 'hpq.us.txt'), delimiter=',', usecols=[
...
# Predict the stock price by using the mean of the last 100 values
...
for pred_idx in range(window_size, N):
    if pred_idx >= N:
        date = dt.datetime.strptime(
            k, '%Y-%m-%d').date() + dt.timedelta(dayss=1)
    else:
...
# Predict stock price using Exponential moving average
